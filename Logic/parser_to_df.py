...
if __name__ == '__main__':
    ...
    #fit_test = ride_parser(fit_file).result
    tcx_test = ride_parser(tcx_file).result
    
    clean_lat = tcx_test["latitude"].dropna()
    clean_long = tcx_test["longitude"].dropna()

    fig = px.line_mapbox(tcx_test, lat="latitude", lon="longitude", hover_name ='distance', center ={'lat':clean_lat.iloc[0], 'lon':clean_long.iloc[0]}, zoom=3, height=600, width =600)
